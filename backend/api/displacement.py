...
from physics import utility_functions, stroke as stroke_module
...
def get_displacement_data():
    ...
    data = state.df_filter[["Zeit [s]", "stroke"]].rename(
        columns={"Zeit [s]": "zeit", "stroke": "stroke"}
    )
    return Response(
...
def apply_displacement_offset():
    ...
    try:
        ...
        data = state.df_filter[cols].rename(
            columns={"Zeit [s]": "zeit", "stroke": "stroke", "stroke_shifted": "stroke_shifted"}
        )
        return Response(
            content=data.to_json(orient="records", double_precision=8),
            media_type="application/json"
...
def apply_displacement_filter(window: int = DEFAULT_FILTER_WINDOW):
    ...
    try:
        ...
        state.df_filter["stroke_filtered"] = utility_functions.filter_vb_style(
        ...
        rename = {
            "Zeit [s]": "zeit", "stroke": "stroke",
            "stroke_shifted": "stroke_shifted", "stroke_filtered": "stroke_filtered"
        }
